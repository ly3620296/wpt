...
<script>
    ...
    layui.use(['form', 'laypage', 'layer', 'table', 'element', 'slider', 'jquery'], function () {
        ...
        table.render({
            ...
            , cols: [[   //表头
                ...
                , {field: 'M_NAME', title: '用户名称'}
                , {field: 'M_ZH', title: '用户账号'}
                , {field: 'M_QX', title: '用户权限'}
                , {fixed: 'right', title: '操作', align: 'center', toolbar: '#barDemo'}
            ]]
            , url: wpt_serverName + 'user/list' //数据接口地址
            , title: '用户表'
            , page: true //开启分页
    ...
    function edit(id) {
        layer.open({
            ...
            title: '编辑用户',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/user/edit.jsp?id='+id,  //调到新增页面
    ...
    function func7() {
        layer.open({
            ...
            title: '添加用户',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/user/add.jsp',  //调到新增页面
            success: function (layero, index) {
                // 新iframe窗口的对象
                var iframeWin = layero.find('iframe')[0].contentWindow;
                // 重新渲染checkbox,select同理
    ...
    function closeAll() {
        layer.closeAll();
        window.location.href = wpt_serverName + '/module/user/list.jsp';
    }

    function del(array) {
        for(var i=0;i<array.length;i++){
            if(array[i]==0){
                layer.msg('不可以删除超级管理员账号');
                return false;
                break;
            }
        }
        $.ajax({
            url: wpt_serverName + "user/del",
            type: 'post',
            dataType: 'json',
            data: {id: array},
            timeout: 10000,
