...
<script>
    ...
    layui.use(['form', 'laypage', 'layer', 'table', 'element', 'slider', 'jquery'], function () {
        ...
        table.render({
            ...
            , url: wpt_serverName + 'zbwh/list' //数据接口地址
            , title: '用户表'
            , page: true //开启分页
    ...
    function edit(id) {
        layer.open({
            ...
            title: '编辑学院',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/zbwh/edit.jsp?id=' + id,  //调到新增页面
    ...
    function detail(id) {
        layer.open({
            ...
            title: '查看值班',
            area: ['70%', '90%'], //宽高
            content: '/wptma/module/zbwh/table.jsp?id='+id,  //调到新增页面
            success: function (layero, index) {
                // 新iframe窗口的对象
                var iframeWin = layero.find('iframe')[0].contentWindow;
                // 重新渲染checkbox,select同理
    ...
    function func7() {
        layer.open({
            ...
            title: '添加值班',
            area: ['70%', '90%'], //宽高
            content: '/wptma/module/zbwh/add.jsp',  //调到新增页面
            success: function (layero, index) {
                // 新iframe窗口的对象
//                var iframeWin = layero.find('iframe')[0].contentWindow;
//                // 重新渲染checkbox,select同理
//                iframeWin.layui.form.render('checkbox');
    ...
    function closeAll() {
        layer.closeAll();
        window.location.href = wpt_serverName + '/module/zbwh/list.jsp';
    }

    function del(array) {
        $.ajax({
            url: wpt_serverName + "zbwh/del",
            type: 'post',
            dataType: 'json',
            data: {id: array},
            timeout: 10000,
