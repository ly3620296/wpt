...
<script>
    ...
    layui.use(['form', 'laypage', 'layer', 'table', 'element', 'slider', 'jquery'], function () {
        ...
        table.render({
            ...
            , cols: [[   //表头
                ...
                , {field: 'X_ID', title: 'ID', sort: true, fixed: 'left'}
                , {field: 'X_NAME', title: '学院名称'}
                , {field: 'X_CODE', title: '学院编码'}
                , {fixed: 'right', title: '操作', align: 'center', toolbar: '#barDemo'}
            ]]
            , url: wpt_serverName + 'xygl/list' //数据接口地址
            , title: '用户表'
            , page: true //开启分页
    ...
    function edit(id) {
        layer.open({
            ...
            title: '编辑学院',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/xygl/edit.jsp?id='+id,  //调到新增页面
    ...
    function func7() {
        layer.open({
            ...
            title: '添加学院',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/xygl/add.jsp',  //调到新增页面
            success: function (layero, index) {
                // 新iframe窗口的对象
                var iframeWin = layero.find('iframe')[0].contentWindow;
                // 重新渲染checkbox,select同理
    ...
    function closeAll() {
        layer.closeAll();
        window.location.href = wpt_serverName + '/module/xygl/list.jsp';
    }

    function del(array) {
        $.ajax({
            url: wpt_serverName + "xygl/del",
            type: 'post',
            dataType: 'json',
            data: {id: array},
            timeout: 10000,
