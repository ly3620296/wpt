...
public class WxPayDao {
    ...
    public void insertOrder(WxPayOrder wxPayOrder, String val) {
        ...
        Db.update(sql, wxPayOrder.getXh(), wxPayOrder.getOut_trade_no(), wxPayOrder.getIds(), wxPayOrder.getPay_type(), Double.parseDouble(wxPayOrder.getTotal_fee()) / 100, wxPayOrder.getAppid(),
                wxPayOrder.getMch_id(), wxPayOrder.getOpenid(), wxPayOrder.getPayIp(), wxPayOrder.getTime_start(), MyWxpayConstant.ORDER_STATE_NOPAY, wxPayOrder.getPREPAY_ID(), wxPayOrder.getSfxn(), wxPayOrder.getOrderNo(), wxPayOrder.getCode_url(), val);
    }


    public void insertOrderYl(WxPayOrder wxPayOrder, String val) {
        String sql = "INSERT INTO WPT_WXZF_SPECIAL_ORDER (XH,OUT_TRADE_NO,IDS,PAY_TYPE,TOTAL_FEE,MCH_ID,PAYIP,TIME_START,ORDER_STATE,SFXN,ORDER_NO,PAY_VAL) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
        Db.update(sql, wxPayOrder.getXh(), wxPayOrder.getOut_trade_no(), wxPayOrder.getIds(), wxPayOrder.getPay_type(), Double.parseDouble(wxPayOrder.getTotal_fee()) / 100,
                wxPayOrder.getMch_id(), wxPayOrder.getPayIp(), wxPayOrder.getTime_start(), MyWxpayConstant.ORDER_STATE_NOPAY, wxPayOrder.getSfxn(), wxPayOrder.getOrderNo(), val);
    }
