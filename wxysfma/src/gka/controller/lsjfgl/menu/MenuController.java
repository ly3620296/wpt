...
public class MenuController extends Controller {
    ...
    public void father() {
        try {
            List<Record> re = Db.find("select M_ID,M_NAME from sfma_menu where m_level='1' and m_state='1'");
            if (re.size() > 0) {
        ...
        try {
            ...
            if (array.length > 0) {
                for (int i = 0; i < array.length; i++) {
                    Db.update("delete sfma_menu where m_id=?", array[i]);
                }
                renderJson(ReturnKit.retOk());
            } else {
    ...
    public void query() {
        try {
            ...
            Record re = Db.findFirst("select * from sfma_menu where m_id=?", id);
            List<Record> list = Db.find("select M_ID,M_NAME from sfma_menu where m_level='1' and m_state='1'");
            if (re != null) {
                map.put("thMenu",re);
