...
public class QxglDao {
    ...
    public static List<Record> list(Integer start, Integer end) {
        String sql = "SELECT q_id,q_name,q_qx,q_ms FROM (SELECT ROWNUM AS rowno,t.*  FROM sfma_qxgl t WHERE ROWNUM <= ? order by to_number(q_id)) a WHERE a.rowno >= ?";
        List<Record> re = Db.find(sql, end, start);
        return re;
    }

    public static Integer searchCount() {
        String sql = "select count(1) from sfma_qxgl";
        Record re = Db.findFirst(sql);
        return re.getInt("count(1)");
    }

    public static Integer add(String q_name, String q_qx, String q_ms) {
        String sql = "insert into sfma_qxgl (q_id,q_name,q_qx,q_ms) values(SEQ_sfma_qxgl.NEXTVAL,?,?,?)";
    ...
    public static String createMenuTree() throws Exception {
        ...
        List<Record> allMenu = Db.find("select m_id,m_name from sfma_menu where m_level='1' and m_state='1' and m_id in(select distinct(m_father) from sfma_menu where m_level='2' and m_state='1')");
        if (allMenu.size() > 0) {
            str.append("[{");
            for (int i = 0; i < allMenu.size(); i++) {
                ...
                List<Record> children = Db.find("select m_id,m_name from sfma_menu where m_level='2' and m_father=? and m_state='1'", m_id);
                if (children.size() > 0) {
                    str.append(",children:[{");
