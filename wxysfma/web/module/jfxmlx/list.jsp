...
<script>
    ...
    layui.use(['form', 'laypage', 'layer', 'table', 'element', 'slider', 'jquery'], function () {
        ...
        table.render({
            ...
            , cols: [[   //表头
                ...
                , {field: 'XMLXID', title: '项目类型ID', sort: true, fixed: 'left'}
                , {field: 'XMLXMC', title: '项目类型名称'}
                , {fixed: 'right', title: '操作', align: 'center', toolbar: '#barDemo'}
            ]]
            , url: wpt_serverName + 'jfxmlx/list' //数据接口地址
            , title: '用户表'
            , page: true //开启分页
            ...
            switch (obj.event) {
                ...
                case 'update':
                    if (data.length === 0) {
                        ...
                        edit(checkStatus.data[0].XMLXID);
                    }
                    break;
                case 'delete':
                    if (data.length === 0) {
    ...
    function edit(id) {
        layer.open({
            ...
            title: '编辑缴费项目',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/jfxmlx/edit.jsp?id=' + id,  //调到新增页面
    ...
    function func7() {
        layer.open({
            ...
            title: '添加缴费项目',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/jfxmlx/add.jsp',  //调到新增页面
            success: function (layero, index) {
                // 新iframe窗口的对象
                var iframeWin = layero.find('iframe')[0].contentWindow;
                // 重新渲染checkbox,select同理
    ...
    function closeAll() {
        layer.closeAll();
        window.location.href = wpt_serverName + '/module/jfxmlx/list.jsp';
    }

    function del(array) {
        $.ajax({
            url: wpt_serverName + "jfxmlx/del",
            type: 'post',
            dataType: 'json',
            data: {id: array},
            timeout: 10000,
