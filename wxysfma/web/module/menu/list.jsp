...
<script>
    ...
    layui.use(['form', 'laypage', 'layer', 'table', 'element', 'slider', 'jquery'], function () {
        ...
        table.render({
            ...
            , cols: [[   //表头
                ...
                , {field: 'M_ID', title: 'ID', sort: true, fixed: 'left'}
                , {field: 'M_LEVEL', title: '菜单级别'}
                , {field: 'M_NAME', title: '菜单名称'}
                , {field: 'M_URL', title: '菜单链接'}
                , {field: 'M_STATE', title: '在线状态', width: 100, color: 'blue'}
                , {fixed: 'right', title: '操作', align: 'center', toolbar: '#barDemo'}
            ]]
            , url: wpt_serverName + 'menu/list' //数据接口地址
            , title: '用户表'
            , page: true //开启分页
    ...
    function edit(id) {
        layer.open({
            ...
            title: '添加菜单',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/menu/edit.jsp?id='+id,  //调到新增页面
    ...
    function func7() {
        layer.open({
            ...
            title: '添加菜单',
            area: ['70%', '68%'], //宽高
            content: '/wptma/module/menu/add.jsp',  //调到新增页面
            success: function (layero, index) {
                // 新iframe窗口的对象
                var iframeWin = layero.find('iframe')[0].contentWindow;
                // 重新渲染checkbox,select同理
    ...
    function closeAll() {
        layer.closeAll();
        window.location.href = wpt_serverName + '/module/menu/list.jsp';
    }

    function del(array) {
        $.ajax({
            url: wpt_serverName + "menu/del",
            type: 'post',
            dataType: 'json',
            data: {id: array},
            timeout: 10000,
